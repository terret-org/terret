...
`{call_id:, verdict: "approved"|"denied", reason?:}`. In M4 nothing parks on
it yet — the resolution machinery is M6 — but the event is contract now, and
subscribers see it like any other durable event.
