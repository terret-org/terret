...
- [x] **Step 4: Run tests and regenerate the catalog**
...
class ProtocolTest < Minitest::Test
  ...
  # On timeout, stop the enclosing task's children first — a StandardError
  # alone doesn't stop siblings (async 2.44.1), so without this a failing
  # await hangs the process instead of failing the test.
  def await(timeout = 5)
...
- [x] **Step 4: Run tests** — file, then `rake test`, green.
...
- [x] **Step 2: Run and verify failure** — `bad_frame` ("set_model is not supported yet").
...
> **Note (review fallout):** the Task 9 quality review surfaced a Critical
> double-overflow bug in `shutdown`; the fix commit added
> `test_repeated_overflow_still_delivers_exactly_one_lagged_error` (a strictly
> stronger version of this task's test) plus a yielding-store replay-race test.
> When executing this task, first check whether the test below adds anything
> beyond what already shipped — if not, mark this task absorbed and move on.
...
- [x] The socket protocol tests pass, including the connection-drop cases (tasks 9-15).
- [x] An agent survives a client disconnect mid-turn (task 14, first test).
- [x] The reconnecting client sees no gap (task 14, second test; demo in task 18).
- [x] `rake test` green, `docs/events.md` current, no invariant relaxed anywhere.
